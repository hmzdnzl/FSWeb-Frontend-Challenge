...
export function GlobalProvider({ children }) {
  ...
  const [profileDataTr, setProfileDataTr] = useState({
    title: "",
    subtitle: "",
    birthLabel: "",
    birth: "",
  ...
  const [projectTitleTr, setProjectTitleTr] = useState("");
  const [projectDataTr, setProjectDataTr] = useState([]);
  const [footerDataTr, setFooterDataTr] = useState({
    title: "",
    text: "",
    mailAddress: "",
    connections: [],
  ...
  useEffect(() => {
    ...
    setProfileDataTr({
      title: data.tr.ProfileSection[0]?.title,
      subtitle: data.tr.ProfileSection[1]?.subtitle,
      birthLabel: data.tr.ProfileSection[2]?.birthLabel,
      birth: data.tr.ProfileSection[3]?.birth,
      addressLabel: data.tr.ProfileSection[4]?.addressLabel,
      address: data.tr.ProfileSection[5]?.address,
      educationLabel: data.tr.ProfileSection[6]?.educationLabel,
      education: data.tr.ProfileSection[7]?.education,
      preferredRoleLabel: data.tr.ProfileSection[8]?.preferredRoleLabel,
      preferredRole: data.tr.ProfileSection[9]?.preferredRole,
      img: data.tr.ProfileSection[10]?.img,
      aboutme: data.tr.ProfileSection[11]?.aboutme,
    });
    setHeaderData({
    ...
    setProjectDataTr(data.tr.ProjectsSection.slice(1));
    setProjectTitle(data.en.ProjectsSection[0]?.title || "Projects");
    setProjectTitleTr(data.tr.ProjectsSection[0]?.title || "Projeler");
    setFooterData({
    ...
    setFooterDataTr({
      title: data.tr.FooterSection.title || "",
      text: data.tr.FooterSection.text || "",
      mailAddress: data.tr.FooterSection.mailAddress || "",
